...
CzARATracker::CzARATracker(std::string strWalletFile)
{
...
CzARATracker::~CzARATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzARATracker::Init()
{
...
bool CzARATracker::Archive(CMintMeta& meta)
{
...
bool CzARATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzARATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzARATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzARATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzARATracker::GetSerialHashes()
{
...
CAmount CzARATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzARATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzARATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzARATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzARATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzARATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzARATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzARATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzARATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzARATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzARATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzARATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzARATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzARATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzARATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzARATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzARATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzara from DB\n", __func__, listDeterministicDB.size());
    }
