...
// Copyright (c) 2016-2018 The Aratriton developers
...
public:
    ...
    bool WriteZARASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZARASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZARASeed_deprecated(uint256& seed);
    bool EraseZARASeed();
    bool EraseZARASeed_deprecated();

    bool WriteZARACount(const uint32_t& nCount);
    bool ReadZARACount(uint32_t& nCount);
