...
CzARAWallet::CzARAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zara seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZARASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZARASeed_deprecated()) {
                    LogPrintf("%s: Updated zARA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zara seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zara wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzARAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZARACount(nCountLastUsed);
    else if (!walletdb.ReadZARACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzARAWallet::Lock()
{
    seedMaster = 0;
}

void CzARAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzARAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZARA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzARAWallet::LoadMintPoolFromDB()
{
...
void CzARAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzARAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzARAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zaraTracker->HasPubcoinHash(pMint.first)) {
...
bool CzARAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZARA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zaraTracker which also adds to database
    pwalletMain->zaraTracker->Add(dMint, true);
...
void CzARAWallet::SeedToZARA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzARAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzARAWallet::GenerateDeterministicZARA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzARAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzARAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
