...
enum ZerocoinSpendStatus {
    ZARA_SPEND_OKAY = 0,                            // No error
    ZARA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZARA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZARA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZARA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZARA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZARA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZARA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZARA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZARA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZARA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZARA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZARA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZARA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZARA_SPENT_USED_ZARA = 14,                      // Coin has already been spend
    ZARA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZARA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZAraBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZARAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzaraAuto) const;
    ...
    */
    ...
    CzARAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzARATracker> zaraTracker;
    ...
    void setZWallet(CzARAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zaraTracker = std::unique_ptr<CzARATracker>(new CzARATracker(strWalletFile));
    }

    CzARAWallet* getZWallet() { return zwalletMain; }
    ...
    /** zARA reset */
    boost::signals2::signal<void()> NotifyzARAReset;
