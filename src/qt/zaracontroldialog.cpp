...
#include "ui_zaracontroldialog.h"
...
std::set<std::string> ZAraControlDialog::setSelectedMints;
std::set<CMintMeta> ZAraControlDialog::setMints;

ZAraControlDialog::ZAraControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZAraControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZAraControlDialog::~ZAraControlDialog()
{
    delete ui;
}

void ZAraControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAraControlDialog::updateList()
{
...
void ZAraControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAraControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAra_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAraControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAraControlDialog::GetSelectedMints()
{
...
void ZAraControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
