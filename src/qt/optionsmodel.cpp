...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ARA);
    ...
    if (!settings.contains("nAnonymizeAratritonAmount"))
        settings.setValue("nAnonymizeAratritonAmount", 1000);

    nAnonymizeAratritonAmount = settings.value("nAnonymizeAratritonAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAratritonAmount"))
        SoftSetArg("-anonymizearatritonamount", settings.value("nAnonymizeAratritonAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in aratriton.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAratritonAmount:
            return QVariant(nAnonymizeAratritonAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAratritonAmount:
            nAnonymizeAratritonAmount = value.toInt();
            settings.setValue("nAnonymizeAratritonAmount", nAnonymizeAratritonAmount);
            emit anonymizeAratritonAmountChanged(nAnonymizeAratritonAmount);
            break;
        case CoinControlFeatures:
