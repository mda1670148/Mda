                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    ui->zARApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzARASyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zARApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzARA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zARA, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzARA_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zARA, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zARA
        sendzARA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zARA
    sendzARA();
}

void PrivacyDialog::on_pushButtonZAraControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZAraControlDialog* zAraControl = new ZAraControlDialog(this);
    zAraControl->setModel(walletModel);
    zAraControl->exec();
}

void PrivacyDialog::setZAraControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAraSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzARA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Aratriton Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zARApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zARA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ARA </b>will be added to the standard transaction fees!<br />";
    ...
    // use mints from zARA selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAraControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZAraControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zARA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zARA"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZARA_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zARA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zARA"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zARA was spent successfully update the addressbook with the label
    ...
    // Clear zara selector in case it was used
    ZAraControlDialog::setSelectedMints.clear();
    ui->labelzAraSelected_int->setText(QString("0"));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zaraTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zARA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zARA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zARA "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zARA "));
    ui->labelzARAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zARA </b> "));
...
{
    ui->labelzARASyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzARA->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zARA
        ui->pushButtonMintzARA->setEnabled(false);
        ui->pushButtonMintzARA->setToolTip(tr("zARA is currently disabled due to maintenance."));

        // Spend zARA
        ui->pushButtonSpendzARA->setEnabled(false);
        ui->pushButtonSpendzARA->setToolTip(tr("zARA is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zARA
        ui->pushButtonMintzARA->setEnabled(true);
        ui->pushButtonMintzARA->setToolTip(tr("PrivacyDialog", "Enter an amount of ARA to convert to zARA", 0));

        // Spend zARA
        ui->pushButtonSpendzARA->setEnabled(true);
        ui->pushButtonSpendzARA->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
