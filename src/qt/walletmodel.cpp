...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered aratriton address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal aratriton:URI (aratriton:XyZ...?message=example)
            {
...
static void NotifyzARAReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzARAReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzARAReset.connect(boost::bind(NotifyzARAReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzARAReset.disconnect(boost::bind(NotifyzARAReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zaraTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
