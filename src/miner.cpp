...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (tx.IsZerocoinSpend()) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zaras that have been in mempool long
					//and higher priority to zaras that are large in value
					...
					// zARA spends can have very large priority, use non-overflowing safe functions
		...
		if (nHeight > 20) {
			...
			if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
				//For the period before v2 activation, zARA will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("AratritonMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake())
			pwalletMain->zaraTracker->RemovePending(pblock->vtx[1].GetHash());
		return error("AratritonMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("AratritonMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("aratriton-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zARA with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					continue;
				}

				//Sign block with the zARA key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zARA key failed \n");
					continue;
				}
			}
		...
		LogPrintf("Running AratritonMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
